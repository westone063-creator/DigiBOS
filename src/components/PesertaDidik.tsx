...
const initialTableData = [
  { nama: 'Ahmad Budi', nipd: '1234', nisn: '0123456789', jk: 'L', tempatLahir: 'Jakarta', tglLahir: '01/05/2012', nik: '3171234567890001', agama: 'Islam', alamat: 'Jl. Merdeka No. 1', rt: '01', rw: '02', dusun: '-', kelurahan: 'Gambir', kecamatan: 'Gambir', kodePos: '10110', rombel: 'Kelas 6', status: 'Naik Kelas', namaAyah: 'Budi Santoso', tahunLahirAyah: '1980', pekerjaanAyah: 'Wiraswasta', nikAyah: '3171234567890002', namaIbu: 'Siti Aminah', tahunLahirIbu: '1982', pekerjaanIbu: 'Ibu Rumah Tangga', nikIbu: '3171234567890003' },
];
...
export default function PesertaDidik() {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "PesertaDidik");
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
    saveAs(blob, `Data_Peserta_Didik.xlsx`);
  };
    ...
    reader.onload = (evt) => {
      ...
      if (bstr) {
        ...
        if (dataImport && dataImport.length > 0) {
           setData((prev: any) => [...prev, ...dataImport]);
        }
      }
    };
  ...
  useEffect(() => {
    localStorage.setItem('dataPesertaDidik', JSON.stringify(data));
  }, [data]);
  ...
  const computedRombelData = useMemo(() => {
    ...
    data.forEach((student: any) => {
      const r = student.rombel || 'Kelas 1';
  ...
  const filteredData = useMemo(() => {
    return data.filter((item: any) => {
      ...
      const matchRombel = rombelFilter === '' || item.rombel === rombelFilter;
      const matchStatus = statusFilter === '' || item.status === statusFilter;
      return matchSearch && matchRombel && matchStatus;
    });
  }, [data, searchTerm, rombelFilter, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-display font-bold text-white mb-1 tracking-wide">Data Peserta Didik</h2>
        <p className="text-slate-400 text-sm">Kelola informasi lengkap data peserta didik</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 rounded-bl-full -mr-8 -mt-8"></div>
          ...
          <div className="text-sm font-medium text-slate-400 mt-1">Total Peserta Didik</div>
        </motion.div>
        ...
        >
          <h3 className="text-sm font-display font-semibold text-white mb-4">Indikator Jenis Kelamin per Rombel</h3>
      ...
      >
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto flex-1">
          ...
          <div className="relative w-full sm:w-auto min-w-[160px]">
            ...
            >
              <option className="bg-slate-800" value="">Semua Rombel</option>
              {computedRombelData.map(r => (
                <option key={r.name} className="bg-slate-800" value={r.name}>{r.name}</option>
              ))}
            </select>
          </div>
          <div className="relative w-full sm:w-auto min-w-[160px]">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto shrink-0">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl backdrop-blur-md bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm font-medium hover:bg-blue-500/20 transition-all shadow-sm">
          ...
          <button onClick={handleAddNew} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 border border-emerald-500/50 text-white text-sm font-medium shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all">
            <Plus className="w-4 h-4" /> <span className="hidden sm:inline">Tambah Peserta Didik</span>
          </button>
        </div>
      </motion.div>
          ...
          <table className="w-full text-left border-collapse min-w-max">
            <thead>
              <tr className="bg-white/5 border-b border-white/10">
                ...
                <th className="py-2.5 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Rombel Saat Ini</th>
                <th className="py-2.5 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider text-center">Status</th>
              </tr>
            </thead>
                  ...
                  <td className="py-2.5 px-3 text-sm text-center whitespace-nowrap">
                    {row.status ? (
                      <span className={`px-2.5 py-1 rounded-md text-[11px] font-medium border ${
                        row.status === 'Naik Kelas' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
