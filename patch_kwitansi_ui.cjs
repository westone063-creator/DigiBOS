...
const selectUI = `                        <div className="flex-1 space-y-1.5">
                          <label className="text-sm font-medium text-slate-300">Pilih Transaksi</label>
                          <select 
                            value={selectedSumberId} 
                            onChange={(e) => handlePilihSumber(e.target.value)}
                            ...
                            {sumberOptions.map((opt: any) => {
                              ...
                              return (
                                <option key={opt.id} value={opt.id}>
                                  {opt.tanggal} - {opt.uraian?.substring(0, 30)}... - Rp {amount?.toLocaleString('id-ID')}
                                </option>
                              );
                            })}
                          </select>
